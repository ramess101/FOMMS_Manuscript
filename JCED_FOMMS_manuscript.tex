...
The overall optimal parameter set is reported in Table \ref{tab:nonbonded params} as the MiPPE cyclohexane parameters. Note that $\lambda_{\rm CH_2} = 16$ is once again found to be the optimal repulsive exponent value. It is important to verify that the overall optimal parameter set does indeed provide accurate predictions of vapor-liquid coexistence properties. For this reason, we perform direct GCMC simulations with the overall optimal parameter set to compare the new MiPPE force field with the initial reference (TraPPE) and the first iteration parameter sets.
