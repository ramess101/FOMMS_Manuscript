...
\subsection{Comparison of GCMC-MBAR and GCMC-HR} \label{sec: GCMC-HR and GCMC-MBAR}
...
1$, and behaves appropriately for intermediate cases. Messerly et al.~\cite{Postdoc_1}propose a heuristic that MBAR estimates are sufficiently reliable when $K^{\rm eff}_{\rm snaps} > 50$ to compute phase equilibria with ITIC. Section \ref{sec: litFF} tests whether this is a reasonable heuristic for GCMC-MBAR as well. 
...
The strong disparity between $K_{\rm snaps}^{\rm eff, vap}$ and $K_{\rm snaps}^{\rm eff, liq}$ when $\lambda_{\rm rr} \neq \lambda_{\rm ref}$ is expected for at least two reasons. The first reason is that the vapor phase has fewer molecules overall. $K_{\rm snaps}^{\rm eff}$ (or alternatively $W_n$) is greatest when the reduced potential energy ($u$ in Equation \ref{eq: reduced potential}) is similar for $\theta_{\rm rr}$ and $\theta_{\rm ref}$. The reduced potential energy is proportional to the total internal energy $(U)$, which is an extensive property that depends on both $N$ and $\theta$. Therefore, even when $\theta_{\rm rr} \not\approx \theta_{\rm ref}$, small values of $N$ lead to $u(\theta_{\rm rr}) \approx u(\theta_{\rm ref})$. 

The second reason is that there are more instances of close range interactions in the liquid phase. Varying $\lambda$ changes the slope and location of the repulsive wall, which greatly impacts the non-bonded energy for close range interactions. These large changes in $U$ when $\lambda_{\rm rr} \neq \lambda_{\rm ref}$ cause $W_n \approx 0$ for the majority of liquid phase configurations. For similar reasons, the overlap in the liquid phase is markedly worse when $\sigma_{\rm rr} \not\approx \sigma_{\rm ref}$. For example, $K_{\rm snaps}^{\rm eff, liq}$ is significantly greater for MiPPE-gen $\Rightarrow$ MiPPE-SL than TraPPE $\Rightarrow$ NERD. 
...
Similar to other alkanes, 16 is once again found to be the optimal repulsive exponent when only considering even integer values. The overall optimal parameter set $(\theta^{\langle2\rangle}$, $\lambda_{\rm CH_2} = 16$) is reported in Table \ref{tab:nonbonded params} as the MiPPE cyclohexane parameters. $\epsilon_{\rm CH_2}$ and $\sigma_{\rm CH_2}$ are reported with three and four digits, respectively, consistent with other MiPPE parameters and to provide a qualitative measure of uncertainty.  
