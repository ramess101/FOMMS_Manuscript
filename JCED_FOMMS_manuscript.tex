...
Similar to other alkanes, 16 is once again found to be the optimal repulsive exponent when only considering even integer values. The overall optimal parameter set $(\theta^{\langle2\rangle}$, $\lambda_{\rm CH_2} = 16$) is reported in Table \ref{tab:nonbonded params} as the MiPPE cyclohexane parameters. The precision of $\epsilon_{\rm CH_2}$ and $\sigma_{\rm CH_2}$ (reported with three and four digits, respectively) is consistent with other MiPPE parameters and provides a qualitative measure of uncertainty.  
